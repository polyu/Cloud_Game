...
static void initSDL()
{
	...
	//=============Video=======================
	screen = SDL_SetVideoMode(RWIDTH, RHEIGHT, 32, SDL_HWSURFACE|SDL_DOUBLEBUF);
	if ( screen == NULL ) {
                ...
                switch( event.type )
				{
                    ...
                    case SDL_KEYDOWN:
						...
						else if(event.key.keysym.sym==SDLK_F3)
						{
							if(!isFullScreen)
							{
								screen = SDL_SetVideoMode(RWIDTH, RHEIGHT, 32, SDL_HWSURFACE|SDL_DOUBLEBUF| SDL_FULLSCREEN );
								isFullScreen=true;
							}
							else
							{
